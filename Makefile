.PHONY: up down logs health build clean-certs clean-app clean-docker certs help
...
certs:
	@bash ./scripts/certs.sh "$(KEYSTORE_FILE)" "$(KEYSTORE_PASSWORD)" "$(KEY_ALIAS)" "$(CERT_PUBLIC_FILE)"

help:
	@echo "Alvos: up, down, logs, reload-nginx, health, build, clean-app, certs, deps, clean-docker"